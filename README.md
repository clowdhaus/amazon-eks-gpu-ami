...
| <a name="input_ami_block_device_mappings"></a> [ami\_block\_device\_mappings](#input\_ami\_block\_device\_mappings) | The block device mappings attached when booting a new instance from the AMI created | `list(map(string))` | <pre>[<br>  {<br>    "delete_on_termination": true,<br>    "device_name": "/dev/xvda",<br>    "volume_size": 20,<br>    "volume_type": "gp3"<br>  }<br>]</pre> | no |
| <a name="input_ami_description"></a> [ami\_description](#input\_ami\_description) | The description to use when creating the AMI | `string` | `"Amazon EKS AL2 GPU image"` | no |
...
| <a name="input_ami_name_prefix"></a> [ami\_name\_prefix](#input\_ami\_name\_prefix) | The prefix to use when creating the AMI name. i.e. - `<ami_name_prefix>-<eks_version>-<timestamp>` | `string` | `"amazon-eks-gpu"` | no |
...
| <a name="input_instance_type"></a> [instance\_type](#input\_instance\_type) | The EC2 instance type to use while building the AMI, such as `m5.large` | `string` | `"g4dn.8xlarge"` | no |
...
| <a name="input_region"></a> [region](#input\_region) | The name of the region, such as us-east-1, in which to launch the EC2 instance to create the AMI | `string` | `"eu-central-1"` | no |
...
| <a name="input_security_group_filter"></a> [security\_group\_filter](#input\_security\_group\_filter) | Filters used to populate the `security_group_ids` field. `security_group_ids` take precedence over this | `map(string)` | `{}` | no |
...
| <a name="input_subnet_filter"></a> [subnet\_filter](#input\_subnet\_filter) | Filters used to populate the subnet\_id field. `subnet_id` take precedence over this | `map(string)` | `{}` | no |
...
| <a name="input_vpc_filter"></a> [vpc\_filter](#input\_vpc\_filter) | Filters used to populate the `vpc_id` field. `vpc_id` take precedence over this | `map(string)` | `{}` | no |
